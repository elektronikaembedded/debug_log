...
struct DefaultOs {
  ...
  static std::uint32_t getTimestamp() { return 0U; }

  static const char *getThreadName() { return "MAIN"; }
};

/**
 * @brief Logger severity level.
 */
enum class LogLevel : std::uint8_t { Error = 0U, Warn, Info, Debug };

/**
 * @brief Application operation status.
 */
enum class OpStatus : std::uint8_t { Ok = 0U, Failed, Skipped, Timeout, None };
 ...
 */
template <typename Transport, typename Config = DefaultConfig,
...
private:
  static void log(OpStatus status, LogLevel level, const char *fmt,
                  va_list args);
